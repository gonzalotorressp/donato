...
export async function downloadClosurePdf(input: ClosurePdfInput) {
  ...
  const doc = new jsPDF({unit:'mm',format:'a4'}); const width=210, margin=15; let y=15;
  const ensure=(needed=12)=>{if(y+needed>282){doc.addPage();y=15;}};
  const line=()=>{doc.setDrawColor(224,229,238);doc.line(margin,y,width-margin,y);y+=5;};
  const section=(title:string)=>{ensure(12);doc.setTextColor(24,63,130);doc.setFont('helvetica','bold');doc.setFontSize(11);doc.text(title,margin,y);y+=4;line();};
  const pair=(label:string,value:string,rightLabel?:string,rightValue?:string)=>{ensure(9);doc.setFontSize(8.5);doc.setFont('helvetica','normal');doc.setTextColor(102,112,133);doc.text(label,margin,y);doc.setFont('helvetica','bold');doc.setTextColor(23,35,61);doc.text(value,margin,y+4);if(rightLabel){doc.setFont('helvetica','normal');doc.setTextColor(102,112,133);doc.text(rightLabel,108,y);doc.setFont('helvetica','bold');doc.setTextColor(23,35,61);doc.text(rightValue||'',108,y+4);}y+=11;};
  const tableRow=(label:string,sigmaValue:string,physicalValue:string,difference:string)=>{ensure(8);doc.setFontSize(8);doc.setFont('helvetica','normal');doc.setTextColor(23,35,61);doc.text(label,margin,y);doc.text(sigmaValue,78,y,{align:'right'});doc.text(physicalValue,132,y,{align:'right'});doc.text(difference,width-margin,y,{align:'right'});y+=7;};
  try { doc.addImage(donatoLogo,'JPEG',margin,y,24,24); } catch {}
  doc.setTextColor(24,63,130);doc.setFont('helvetica','bold');doc.setFontSize(20);doc.text('DONATO',45,y+7);doc.setFontSize(13);doc.text('Resumen de cierre de caja',45,y+15);doc.setFont('helvetica','normal');doc.setFontSize(8.5);doc.setTextColor(102,112,133);doc.text(`Generado por ${input.generatedBy}`,45,y+21);y+=31;
