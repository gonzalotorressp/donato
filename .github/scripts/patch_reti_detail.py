...
old = "  snapshot.cuentaCorrienteDocumentos.sort((a, b) => String(a.hora).localeCompare(String(b.hora)) || String(a.key).localeCompare(String(b.key)));\n  return snapshot;"
new = "  snapshot.cuentaCorrienteDocumentos.sort((a, b) => String(a.hora).localeCompare(String(b.hora)) || String(a.key).localeCompare(String(b.key)));\n  snapshot.retirosDocumentos.sort((a, b) => Number(b.importe || 0) - Number(a.importe || 0) || String(a.usuarioNombre || '').localeCompare(String(b.usuarioNombre || '')));\n  return snapshot;"
if old not in s:
    raise SystemExit('snapshot sort block not found')
s = s.replace(old, new, 1)
...
if old not in s:
    raise SystemExit('SigmaSnapshot type target not found')
s = s.replace(old, new, 1)
